...
# https://learn.microsoft.com/en-us/powershell/azure/install-azure-powershell?view=azps-11.4.0
Update-Module Az
...
curl -X POST --data "$BODY" "$spoke3/api/commands" # Deny

BODY=$(echo "HTTP GET \"https://www.microsoft.com\"")
curl -X POST --data "$BODY" "$spoke1/api/commands" # OK (via firewall)
curl -X POST --data "$BODY" "$spoke2/api/commands" # OK (via firewall)
curl -X POST --data "$BODY" "$spoke3/api/commands" # OK (via firewall)
...
curl -X POST --data  "HTTP GET \"$spoke2\"" "$spoke3/api/commands" # Deny
